...
class EmployeeController extends Controller
{
    ...
    {
        \Log::info('Employee store called', [
            'all_data' => array_diff_key($request->all(), ['documents' => '']),
            'has_permission' => Auth::user()->can('create-employees'),
            'user_id' => Auth::id(),
        ]);
    ...
    public function edit(Employee $employee)
    {
        if (Auth::user()->can('edit-employees')) {
            ...
            return Inertia::render('Hrm/Employees/Edit', [
                ...
                'employees' => Employee::with('user')->where('created_by', creatorId())->where('id', '!=', $employee->id)->get()->map(function ($emp) {
                    return [
                        'id' => $emp->id,
                        'name' => $emp->user->name ?? 'Unknown',
                    ];
