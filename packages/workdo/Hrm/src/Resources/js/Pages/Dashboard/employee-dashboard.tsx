            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="h-80 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100 space-y-3 pr-2">
                                {stats.recent_payslips && stats.recent_payslips.length > 0 ? (
                                    stats.recent_payslips.map((payslip) => (
                                        <div key={payslip.id} className="flex items-center justify-between p-3 bg-white rounded-lg border border-gray-200 hover:border-gray-300 transition-colors">
                                            <div className="min-w-0">
                                                <p className="text-sm font-bold text-gray-800 truncate">{payslip.payroll_name}</p>
                                                <p className="text-xs text-gray-500 mt-0.5">{t('Net Pay')}: <span className="font-bold text-emerald-600">{payslip.net_pay}</span></p>
                                                <p className="text-[10px] text-gray-400">{formatDate(payslip.created_at)}</p>
                                            </div>
                                            <Button 
                                                size="sm" 
                                                variant="outline" 
                                                className="h-8 text-xs shrink-0 bg-slate-50 hover:bg-slate-100"
                                                onClick={() => window.open(route('hrm.payroll-entries.print', payslip.id), '_blank')}
                                            >
                                                {t('Print')}
                                            </Button>
                                        </div>
                                    ...
                                    <div className="flex items-center justify-center h-40 text-gray-500">
                                        <div className="text-center">
                                            ...
                                            <p className="text-sm">{t('No payslips available')}</p>
                                        </div>
                                    </div>
                                )}
                            </div>
