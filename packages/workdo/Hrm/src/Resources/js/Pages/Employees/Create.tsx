...
export default function Create() {
    const { users, branches, departments, designations, shifts, documentTypes, generatedEmployeeId, employees } = usePage<any>().props;
    ...
    const submit = (e: React.FormEvent) => {
        ...
        const skip = ['documents'];
        Object.keys(data).forEach((key) => {
            if (!skip.includes(key) && data[key] !== null && data[key] !== undefined) {
                formData.append(key, data[key]);
            }
        });

        // Append document rows that have at least a file or type
        let docIndex = 0;
        (data.documents || []).forEach((doc: any) => {
            if (doc.document_type_id || doc.file) {
                if (doc.document_type_id) {
                    formData.append(`documents[${docIndex}][document_type_id]`, doc.document_type_id);
                }
        ...
        router.post(route('hrm.employees.store'), formData, {
            forceFormData: true,
            preserveState: true,
            preserveScroll: true,
            onSuccess: () => {
                        ...
                        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                            ...
                            <TabsContent value="employment" className="space-y-6 mt-6">
                                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                                    ...
                                    <div>
                                        <Label htmlFor="manager_id">{t('Reporting Manager')}</Label>
                                        <Select
                                        ...
                                        >
                                            ...
                                            <SelectContent searchable={true}>
                                                <SelectItem value="none">{t('None (Top Level Manager)')}</SelectItem>
                            ...
                            <TabsContent value="documents" className="space-y-6 mt-6">
                                <div className="flex justify-between items-center">
                                    <div>
                                        ...
                                        <p className="text-sm text-muted-foreground">{t('Optional — documents can be added now or later.')}</p>
                                    </div>
