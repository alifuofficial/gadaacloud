...
export default function Landing({ settings }: LandingProps) {
    ...
    const sectionOrder = settings?.config_sections?.section_order ||
        ['header', 'hero', 'stats', 'features', 'modules', 'industries', 'benefits', 'testimonials', 'gallery', 'faq', 'cta', 'footer'];
    ...
    const renderSection = useCallback((sectionKey: string) => {
        ...
        switch (sectionKey) {
            ...
            case 'industries':
                return <Industries key={sectionKey} settings={settings} />;
            case 'benefits':
                return <Benefits key={sectionKey} settings={settings} />;
            case 'testimonials':
                return <Testimonials key={sectionKey} settings={settings} />;
            case 'gallery':
                return <Gallery key={sectionKey} settings={settings} />;
            case 'faq':
                return <FAQ key={sectionKey} settings={settings} />;
            case 'cta':
    ...
    return (
        <div className="min-h-screen bg-white">
            <Head title={`${settings?.company_name || 'GadaaCloud ERP'} — All-in-One ERP for Ethiopian Business`}>
