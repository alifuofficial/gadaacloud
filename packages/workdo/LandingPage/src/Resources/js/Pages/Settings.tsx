...
export default function Settings({ settings, customPages }: SettingsProps) {
    ...
    const DEFAULT_VISIBILITY = {
        header: true, hero: true, stats: true, features: true, modules: true,
        industries: true, benefits: true, testimonials: true, gallery: true,
        faq: true, cta: true, footer: true, pricing: true
    };
    const DEFAULT_ORDER = ['header', 'hero', 'stats', 'features', 'modules', 'industries', 'benefits', 'testimonials', 'gallery', 'faq', 'cta', 'footer'];

    const incomingConfig = settings.config_sections || {
        sections: {},
        section_visibility: { ...DEFAULT_VISIBILITY },
        section_order: [...DEFAULT_ORDER]
    };

    // Merge defaults onto what came from DB so newly-introduced sections
    // (industries / testimonials / faq) never get silently dropped on save.
    const mergedSectionVisibility = { ...DEFAULT_VISIBILITY, ...(incomingConfig.section_visibility || {}) };
    const incomingOrder = incomingConfig.section_order || [];
    const mergedSectionOrder = Array.from(new Set([...DEFAULT_ORDER, ...incomingOrder]));
    ...
    const { data, setData, post, put, processing, reset } = useForm({
        ...
        config_sections: {
            ...
            section_order: mergedSectionOrder
        }
    });
