...
export default function Footer({ settings }: FooterProps) {
    ...
    const description = sectionData.description || 'The all-in-one ERP, Accounting, CRM, POS & HRM platform built for Ethiopian businesses — run operations in Birr, Amharic and on cloud.';
    const contactEmail = settings?.contact_email || 'hello@erpgosaas.et';
    const phone = settings?.contact_phone || '+251 11 553 0000';
    const newsletterTitle = sectionData.newsletter_title || 'Join the Ethiopian Business Community';
    const newsletterDescription = sectionData.newsletter_description || 'Get product updates, tax tips and growth stories for Ethiopian SMEs — straight to your inbox.';
    const newsletterButtonText = sectionData.newsletter_button_text || 'Subscribe';
    const copyrightText = sectionData.copyright_text || `© ${new Date().getFullYear()} ${companyName}. Made in Ethiopia. All rights reserved.`;
