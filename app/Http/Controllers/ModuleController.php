...
class ModuleController extends Controller
{
    ...
    /**
     * Superadmin-only flat listing of every installed module/add-on.
     * Gate is the 'superadmin' role (not just the 'manage-add-on' permission)
     * so company admins cannot reach this overview page.
     */
    public function list()
    {
        ...
        // Auto-discover filesystem packages and register them in the addons database table
                ...
                if (File::exists($moduleJsonPath)) {
                    ...
                    if ($moduleData && isset($moduleData['name'])) {
                        ...
                        if (!$addon) {
                            ...
                            $addon->name = $moduleData['alias'] ?? $moduleData['name'];
                            $addon->monthly_price = $moduleData['monthly_price'] ?? 0;
                            $addon->yearly_price = $moduleData['yearly_price'] ?? 0;
                            $addon->package_name = $moduleData['package_name'] ?? strtolower($moduleData['name']);
                            $addon->for_admin = $moduleData['for_admin'] ?? false;
                            $addon->priority = $moduleData['priority'] ?? 0;
                            $addon->is_enable = false; // default disabled
                            $addon->save();
                        }
                    }
                }
        ...
        $addons = AddOn::orderBy('priority', 'asc')
            ->orderBy('name', 'asc')
            ->get()
            ->map(function ($addon) {
                return [
                    ...
                    'module'        => $addon->module,
                    'name'          => $addon->name,
                    'image'         => url('/packages/workdo/' . $addon->module . '/favicon.png'),
                    'monthly_price' => (float) $addon->monthly_price,
                    'yearly_price'  => (float) $addon->yearly_price,
                    'is_enable'     => (bool) $addon->is_enable,
                    'for_admin'     => (bool) $addon->for_admin,
                    'package_name'  => $addon->package_name,
                    'priority'      => (int) $addon->priority,
                ];
            });

        $enabled  = $addons->where('is_enable', true)->count();
        $disabled = $addons->where('is_enable', false)->count();
        $adminOnly = $addons->where('for_admin', true)->count();
    ...
    /**
     * Update monthly and yearly price for a specific module.
     * Superadmin only.
     */
    public function updatePrice(Request $request, string $moduleName)
    {
        if (!Auth::user()->hasRole('superadmin')) {
            abort(403, __('Permission denied'));
        }
        ...
        $addon = AddOn::where('module', $moduleName)->firstOrFail();
        $addon->monthly_price = $request->monthly_price;
        $addon->yearly_price  = $request->yearly_price;
        $addon->save();

        return redirect()->back()->with('success', __('Module pricing updated successfully.'));
    }
